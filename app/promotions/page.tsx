...
function PromotionCard({ event, interested, saved, booked, onToggleInterest, onToggleSave, onToggleBooked }: { event: PromotionEvent; interested: boolean; saved: boolean; booked: boolean; onToggleInterest: () => void; onToggleSave: () => void; onToggleBooked: () => void; }) {
  ...
  return (
    <div className="rounded-3xl border border-gray-100 bg-white p-4 shadow-sm">
      ...
      <div className="mt-3 flex flex-wrap gap-2">{event.tags.map((tag) => <span key={tag} className="rounded-full bg-gray-100 px-2.5 py-1 text-[11px] text-gray-500">#{lt(tag)}</span>)}</div>
...
export default function PromotionsPage() {
  ...
  const { lt } = useLocalizedText();
  const visibleEvents = useMemo(() => filter === "all" ? promotionEvents : promotionEvents.filter((event) => event.status === filter), [filter]);
  ...
  return (
    <AppShell>
      ...
      <div className="space-y-4 px-4 py-4">
        ...
        <section className="space-y-3">{visibleEvents.map((event) => <PromotionCard key={event.id} event={event} interested={interestedPromotionIds.includes(event.id)} saved={savedPromotionIds.includes(event.id)} booked={bookedPromotionIds.includes(event.id)} onToggleInterest={() => togglePromotionInterest(event.id)} onToggleSave={() => handleToggleSchedule(event)} onToggleBooked={() => toggleBookedPromotion(event.id)} />)}</section>
      </div>
    </AppShell>
  );
}
