...
export function getCareTriageResult(input: CareTriageInput): CareTriageResult {
  if (input.isBreathingIssue || input.isHeavyBleeding || input.hasChestPain || input.severityLevel === "severe") {
    return {
      ...
      summary: "This situation may need emergency evaluation first. Use 119 or go to an emergency-ready hospital immediately.",
      reasons: [
      ...
      nextActions: ["call-119", "call-1339", "find-clinic"],
    };
  }
  ...
  if (input.symptomCategory === "mental-health") {
    return {
      ...
      nextActions: ["find-specialist", "open-1330"],
    };
  }
  ...
  if (input.isNightTime && input.severityLevel === "mild") {
    return {
      level: "pharmacy",
      title: "Start with a pharmacy",
      summary: "For mild issues at night, a pharmacy may be the quickest next step before deciding on a clinic visit.",
      reasons: ["Night-time + mild symptoms"],
      nextActions: ["find-pharmacy", "call-1339"],
    };
  }
  ...
  if (input.symptomCategory === "fever-cold" && input.severityLevel === "moderate") {
    return {
      ...
      nextActions: ["call-1339", "find-clinic"],
    };
  }
  ...
  if (input.symptomCategory === "stomach" || input.symptomCategory === "injury" || input.symptomCategory === "other") {
    return {
      ...
      nextActions: ["find-clinic", "open-1330"],
    };
  }
  ...
  return {
    ...
    nextActions: ["find-pharmacy", "find-clinic"],
  };
}
