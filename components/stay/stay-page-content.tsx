...
import { StaySetupTimeline } from "@/components/stay/stay-setup-timeline";
import { StayProgressSnapshot } from "@/components/stay/stay-progress-snapshot";
import { StaySettlementCommandCenter } from "@/components/stay/stay-settlement-command-center";
import { StayChecklistPriorityLanes } from "@/components/stay/stay-checklist-priority-lanes";
import { StayDocumentStarter } from "@/components/stay/stay-document-starter";
    ...
    <AppShell>
      ...
      <div className="px-4 py-4">
        <Tabs value={tabValue} onValueChange={handleTabChange} className="gap-4">
          ...
          <TabsContent value="overview" className="space-y-4"><StaySettlementCommandCenter input={draft} /><StayProgressSnapshot input={draft} /><TrustLayerPanel description="Landly Stay marks immigration, insurance, housing, labor, and student resources by trust level before you open the official route." /><StayQuickActions /><StaySetupTimeline input={draft} /><StayDeadlineCards reminders={summary.reminders} /><div className="rounded-2xl border border-gray-100 bg-white p-4 text-sm leading-relaxed text-gray-600 shadow-sm"><p className="font-semibold text-gray-900">{lt("How Landly Stay helps")}</p><p className="mt-2">{lt("Landly Stay turns official immigration, insurance, housing, labor, and student resources into a clearer setup flow. Use it to decide what to do now, what to save, and where to go for official support.")}</p></div></TabsContent>
          <TabsContent value="first90" className="space-y-4"><StaySetupTimeline input={draft} /><Stay90DayMissions stayType={draft.stayType} /></TabsContent>
          <TabsContent value="plan" className="space-y-4"><StaySettlementCommandCenter input={draft} /><StayPlanBuilder value={draft} onChange={(patch) => { const next = { ...draft, ...patch }; setDraft(next); saveStayPlanInput(next); }} /><StayPlanSummary immediateTasks={immediateTasks} thisMonthTasks={thisMonthTasks} resources={resources} reminders={summary.reminders} /></TabsContent>
          <TabsContent value="checklist" className="space-y-4"><StayChecklistPriorityLanes input={draft} activeCategory={activeCategory} onCategoryChange={handleChecklistCategoryChange} /><LifeChecklist category={activeCategory} onCategoryChange={handleChecklistCategoryChange} /></TabsContent>
          <TabsContent value="documents" className="space-y-4"><StayDocumentStarter input={draft} /><StayDocumentVault /></TabsContent>
